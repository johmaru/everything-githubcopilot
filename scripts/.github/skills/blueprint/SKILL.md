...
- **Zero runtime risk** — Pure Markdown skill. The entire repository contains only `.md` files — no hooks, no shell scripts, no executable code, no `package.json`, no build step. Nothing runs on install or invocation beyond GitHub Copilot's native Markdown skill loader.

## Installation

This skill ships with Everything GitHub Copilot. No separate installation is needed when EGP is installed.

### Full EGP install

If you are working from the EGP repository checkout, verify the skill is present with:

```bash
test -f skills/blueprint/SKILL.md
```

To update later, review the EGP diff before updating:
...
If you are vendoring only this skill outside the full EGP install, copy the reviewed file from the EGP repository into `.github/skills/`blueprint/SKILL.md`. Vendored copies do not have a git remote, so update them by re-copying the file from a reviewed EGP commit rather than running `git pull`.

## Requirements

- GitHub Copilot (for `/blueprint` slash command)
