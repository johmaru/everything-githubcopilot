...
Integration reference for Plankton (credit: @alxfazio), a write-time code quality enforcement system for GitHub Copilot. Plankton runs formatters and linters on every file edit via PostToolUse hooks, then spawns Claude subprocesses to fix violations the agent didn't catch.
...
Every time GitHub Copilot edits or writes a file, Plankton's `multi_linter.sh` PostToolUse hook runs:
...
├─ Spawns copilot-cli subprocess with violations JSON
...
# Start GitHub Copilot — hooks activate automatically
claude
```
...
1. Install EGP as your plugin (agents, skills, commands, rules)
2. Add Plankton hooks for write-time quality enforcement
3. Use AgentShield for security audits
4. Use EGP's verification-loop as a final gate before PRs

### Avoiding Hook Conflicts

If running both EGP and Plankton hooks:
- EGP's Prettier hook and Plankton's biome formatter may conflict on JS/TS files
- Resolution: disable EGP's Prettier PostToolUse hook when using Plankton (Plankton's biome is more comprehensive)
- Both can coexist on different file types (EGP handles what Plankton doesn't cover)
