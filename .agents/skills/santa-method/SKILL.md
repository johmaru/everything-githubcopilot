...
```python
# Spawn reviewers in parallel (GitHub Copilot subagents)
...
### Pattern A: GitHub Copilot Subagents (Recommended)

Subagents provide true context isolation. Each reviewer is a separate process with no shared state.

```bash
# In a GitHub Copilot session, use the Agent tool to spawn reviewers
