...
注: user-level installer は VS Code Copilot 向けです。Codex CLI 用の `.codex/`、project-local な `.agents/skills/` ブリッジ、`.codex/skills/` 互換 alias は project setup で各 project に配布してください。`~/.codex/skills` は管理対象外なので、そこに出る invalid `SKILL.md` 警告は user 環境側で修正または削除する必要があります。
