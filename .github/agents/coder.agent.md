...
- **High-risk edit直後に安全性チェックを省略しない。** workspace instructions、agents、file instructions、prompts、skills、hooks、installer scripts、package metadata、validators、schemas、security-sensitive automation、削除、上書き、設定変更、または複数ファイル変更を含む場合は safety-checker を呼び、必要なら一時バックアップを作成してから検証へ進む。通常の単一ファイル実装変更で、これらの条件に当てはまらない場合は省略してよい。
- **推測でコードを書かない。** 不明な仕様は
...
3. **High-risk edit では即座に safety-checker を呼ぶ**。危険箇所があれば `scripts/hooks/safety-backup.js` を使って一時バックアップを作成する。`session id` は既存の値をそのまま使い、`file` は変更済みファイル一覧にある repo 内パスだけ、`reason` は `settings-risk` のような短い slug だけを渡す。要約文や改行入り文字列をコマンドに埋め込まない。通常の単一ファイル実装変更や low-risk なテスト更新で、workspace instructions、agents、file instructions、prompts、skills、hooks、installer scripts、package metadata、validators、schemas、security-sensitive automation、削除、上書き、設定変更、複数ファイル変更のいずれにも当てはまらない場合は省略してよい
...
### Phase 2.6 — Safety Checker呼び出し（High-risk editで必須）

`Edit` / `Write` / `MultiEdit` を行った直後に、high-risk 条件へ当てはまる場合だけ safety-checker を agent ツールで呼び出してください。

**呼び出し条件:**

- workspace instructions、agents、file instructions、prompts、skills、hooks、installer、package metadata、validators、schemas、security-sensitive automation を変更した時
- 1回の編集で複数ファイルを触った時
- 削除、上書き、設定変更、フック変更を含む時
- 通常の単一ファイル実装変更、軽微な文言修正、ローカライズ、または低リスクなテスト更新で上記条件に当てはまらない時は省略可
...
- ✅/❌ 回帰チェック → 各ステップでは変更面に近い回帰確認を行い、広い回帰確認は最終確認または high-risk closeout で実行する
...
最終確認が通ったら、変更リスクに応じて review 経路を選んでください:

- **Low-risk 直接完了**: 単一ファイルの軽微な文言修正、ローカライズ、または低リスクなテスト更新で、workspace instructions、agents、file instructions、prompts、skills、hooks、installer scripts、package metadata、validators、schemas、security-sensitive automation のいずれにも触れていない場合だけ `(完了)` を使ってよい
- **既定 review**: 実装タスクの通常系は `researcher` に handoff して差分の正しさ、回帰、リスクを確認する
- **High-risk 追加 review**: workspace instructions、agents、file instructions、prompts、skills、hooks、installer scripts、package metadata、validators、schemas、または security-sensitive automation を触った場合は、`code-reviewer` を追加で呼び、Copilot discovery、frontmatter、validation gap も確認する

High-risk review を追加した場合でも、planner -> coder -> researcher の実装レーンは維持してください。
`code-reviewer` は handoff ではなく `agent` ツールで明示的に呼び出し、変更ファイル一覧、何を変えたか、high-risk と判定した理由を渡してください。
...
単一ファイルの low-risk 変更で、workspace instructions、agents、file instructions、prompts、skills、hooks、installer scripts、package metadata、validators、schemas、security-sensitive automation のいずれにも触れていない場合だけ `(完了)` を使います。
High-risk 変更では、researcher handoff の前に `code-reviewer` を追加で呼び出してから完了ブロックへ進んでください。
