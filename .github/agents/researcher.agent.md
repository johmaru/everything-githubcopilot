...
- repo 内の依存関係追跡、rename 影響調査、dead export 確認、責務の横断把握では `#search/usages` を主手段として使い、`#search/textSearch` は生文字列やコメント探索の補助に留める。
- text hit を見つけても、実際の参照関係や呼び出し経路は `#search/usages` で裏取りしてから結論を書く。
