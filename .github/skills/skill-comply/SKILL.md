...
3. Running `copilot-cli` and capturing tool call traces via stream-json
...
```bash
# Full run
uv run python -m scripts.run `.github/instructions/`common/testing.md

# Dry run (no cost, spec + scenarios only)
uv run python -m scripts.run --dry-run `.github/skills/`search-first/SKILL.md
