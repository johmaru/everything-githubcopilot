...
Patterns, architectures, and reference implementations for running GitHub Copilot autonomously in loops. Covers everything from simple `copilot-cli` pipelines to full RFC-driven multi-agent DAG orchestration.
...
## 1. Sequential Pipeline (`copilot-cli`)

**The simplest loop.** Break daily development into a sequence of non-interactive `copilot-cli` calls. Each call is a focused step with a clear prompt.

### Core Insight

> If you can't figure out a loop like this, it means you can't even drive the LLM to fix your code in interactive mode.

The `copilot-cli` flag runs GitHub Copilot non-interactively with a prompt, exits when done. Chain calls to build a pipeline:
...
copilot-cli "Read the spec in docs/auth-spec.md. Implement OAuth2 login in src/auth/. Write tests first (TDD). Do NOT create any new documentation files."

# Step 2: De-sloppify (cleanup pass)
copilot-cli "Review all files changed by the previous commit. Remove any unnecessary type tests, overly defensive checks, or testing of language features (e.g., testing that TypeScript generics work). Keep real business logic tests. Run the test suite after cleanup."

# Step 3: Verify
copilot-cli "Run the full build, lint, type check, and test suite. Fix any failures. Do not add new features."

# Step 4: Commit
copilot-cli "Create a conventional commit for all staged changes. Use 'feat: add OAuth2 login flow' as the message."
```

### Key Design Principles

1. **Each step is isolated** — A fresh context window per `copilot-cli` call means no context bleed between steps.
...
copilot-cli --model opus "Analyze the codebase architecture and write a plan for adding caching..."

# Implement with Sonnet (fast, capable)
copilot-cli "Implement the caching layer according to the plan in docs/caching-plan.md..."

# Review with Opus (thorough)
copilot-cli --model opus "Review all changes for security issues, race conditions, and edge cases..."
```
...
copilot-cli "Read .claude-context.md for priorities. Work through them in order."
...
copilot-cli --allowedTools "Read,Grep,Glob" "Audit this codebase for security vulnerabilities..."

# Write-only implementation pass
copilot-cli --allowedTools "Read,Write,Edit,Bash" "Implement the fixes from security-audit.md..."
```
...
**EGP's built-in persistent loop.** A session-aware REPL that calls `copilot-cli` synchronously with full conversation history.
...
1. Loads conversation history from `~/.github/claw/{session}.md`
2. Each user message is sent to `copilot-cli` with full history as context
...
### Implementation via GitHub Copilot Commands

Create `.github/commands/infinite.md`:
...
**A production-grade shell script** that runs GitHub Copilot in a continuous loop, creating PRs, waiting for CI, and merging automatically. Created by AnandChowdhary (credit: @AnandChowdhary).
...
│  2. Run copilot-cli with enhanced prompt              │
│  3. (Optional) Reviewer pass — separate copilot-cli   │
│  4. Commit changes (claude generates message)       │
│  5. Push + create PR (gh pr create)                 │
│  6. Wait for CI checks (poll gh pr checks)          │
│  7. CI failure? → Auto-fix pass (copilot-cli)         │
...
Claude reads this file at iteration start and updates it at iteration end. This bridges the context gap between independent `copilot-cli` invocations.

### CI Failure Recovery

When PR checks fail, Continuous Claude automatically:
1. Fetches the failed run ID via `gh run list`
2. Spawns a new `copilot-cli` with CI fix context
...
copilot-cli "Implement the feature with full TDD. Be thorough with tests."

# Step 2: De-sloppify (separate context, focused cleanup)
copilot-cli "Review all changes in the working tree. Remove:
...
for feature in "${features[@]}"; do
  # Implement
  copilot-cli "Implement $feature with TDD."

  # De-sloppify
  copilot-cli "Cleanup pass: review changes, remove test/code slop, run tests."

  # Verify
  copilot-cli "Run build + lint + tests. Fix any failures."

  # Commit
  copilot-cli "Commit with message: feat: add $feature"
done
```
...
3. **Any loop + Verification** — Use EGP's `/verify` command or `verification-loop` skill as a gate before commits.
...
4. **Ralphinho's tiered approach in simpler loops** — Even in a sequential pipeline, you can route simple tasks to Haiku and complex tasks to Opus:
   ...
   copilot-cli --model haiku "Fix the import ordering in src/utils.ts"

   # Complex architectural change
   copilot-cli --model opus "Refactor the auth module to use the strategy pattern"
   ```
...
2. **No context bridge between iterations** — Each `copilot-cli` call starts fresh. Use `SHARED_TASK_NOTES.md` or filesystem state to bridge context.
...
| NanoClaw | EGP | `/claw` command in this repo |
| Verification Loop | EGP | `skills/verification-loop/` in this repo |
